...
@app.post('/login')
async def perform_login(request: Request,login_type:str = Form(...), username:str= Form(...),password:str= Form(...)):
...
@app.post('/add_item')
async def add_item(request:Request,item_name:str = Form(...),item_price:str = Form(...),item_url:str = Form(...)):
...
async def get_item_page(request:Request):
    ...
    print(index)
    return templates.TemplateResponse(f"/item_page.html",{"request":request,"username":ACTIVE_USER,"index":index,**item})
...
async def client_home(request:Request):
    print(ACTIVE_USER)
    return templates.TemplateResponse("client_home.html",{"request": request,"auction_items": AUCTION_ITEMS,"username":ACTIVE_USER})
